            ...
            <div className='grid h-full md:min-h-screen lg:grid-cols-[280px_1fr] w-full'>
                <div className='bg-card bg-cover bg-center'>
                    <ul className='flex items-start p-4 justify-center md:flex-col gap-6 md:gap-4 mt-2 font-bold text-sm'>
                        <li className={selectedFilter === 'PENDING' ? 'transition-all  flex items-center gap-2 hover:text-muted cursor-pointer text-yellow-300' : 'transition-all  flex items-center gap-2 hover:text-muted cursor-pointer'} onClick={() => {
                            setSelectedFilter('PENDING');
                        }}>
                            <Icons.pending />
                            Pending
                        </li>
                        <li className={selectedFilter === 'COMPLETED' ? 'transition-all  flex items-center gap-2 hover:text-muted cursor-pointer text-yellow-300' : 'transition-all  flex items-center gap-2 hover:text-muted cursor-pointer'} onClick={() => {
                            setSelectedFilter('COMPLETED');
                        }}>
                            <Icons.completed />
                            Completed
                        </li>
                        <li className={selectedFilter === 'ALL' ? 'transition-all  flex items-center gap-2 hover:text-muted cursor-pointer text-yellow-300' : 'transition-all  flex items-center gap-2 hover:text-muted cursor-pointer'} onClick={() => {
                        ...
                        {filteredOrder?.map((order) => {
                            return (
                                <TableRow key={order.id}>
                                    ...
                                    <TableCell>
                                        {order.status === 'PENDING' ? (<CraftModal img={order.item?.Image} player={order.user.name} id={order.id} name={order.item?.name} detaiils={order.details} enchant={order.Enchants} />) : <p>Nothing here yet...</p>}
                                    </TableCell>
                                </TableRow>
                            )
                        })}
